...
func (r TransactionRequest) Validate() *customErrors.AppError {
	if !r.IsTransactionTypeWithdrawal() && !r.IsTransactionTypeDeposit() {
		return customErrors.NewValidationError("Transaction type can only be deposit or withdrawal")
	}
	if r.Amount < 0 {
		return customErrors.NewValidationError("Amount cannot be less than zero")
	}
	return nil
}
